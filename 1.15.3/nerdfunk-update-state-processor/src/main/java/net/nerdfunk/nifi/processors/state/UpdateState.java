...
@CapabilityDescription("Gets either the content of a FlowFile or the value of a specified attribute and puts it to a distributed map cache, using a cache key " +
...
public class UpdateState extends AbstractProcessor {
    ...
    // Set strategy. Either use content or attribute to set state
    public static final AllowableValue CS_CONTENT = new AllowableValue("content", "Content",
    "Use content to update cache.");
    public static final AllowableValue CS_ATTRIBUTE = new AllowableValue("attribute", "Attribute",
    "Use attribute to update cache.");
    public static final PropertyDescriptor CACHE_SOURCE_STRATEGY = new PropertyDescriptor
            .Builder().name("Cache source Strategy")
            .description("Use either attribute or content of flowfile")
            .required(true)
            .allowableValues(CS_CONTENT, CS_ATTRIBUTE)
            .defaultValue(CS_CONTENT.getValue())
            .build();

    // Selects the FlowFile attribute, whose value is used as cache key
    public static final PropertyDescriptor ATTRIBUTE_IDENTIFIER = new PropertyDescriptor.Builder()
        .name("Attribute Identifier")
        .description("Name of the attribute to use to update state")
        .required(true)
        .addValidator(StandardValidators.createAttributeExpressionLanguageValidator(ResultType.STRING, true))
        .expressionLanguageSupported(ExpressionLanguageScope.FLOWFILE_ATTRIBUTES)
        .build();
    ...
    protected List<PropertyDescriptor> getSupportedPropertyDescriptors() {
        ...
        descriptors.add(CACHE_SOURCE_STRATEGY);
        descriptors.add(ATTRIBUTE_IDENTIFIER);
    ...
    public void onTrigger(final ProcessContext context, final ProcessSession session) throws ProcessException {
        ...
        final String sourceStrategy = context.getProperty(CACHE_SOURCE_STRATEGY).getValue();
        final String updateStrategy = context.getProperty(CACHE_UPDATE_STRATEGY).getValue();
        boolean cached = false;
        byte[] cacheValue = null;

        if (sourceStrategy.equals(CS_ATTRIBUTE.getValue())) {
            // get name of attribute we use to get value
            final String cs_attribute = context.getProperty(ATTRIBUTE_IDENTIFIER).evaluateAttributeExpressions(flowFile).getValue();
            cacheValue = flowFile.getAttribute(cs_attribute).getBytes();
            //String s = new String(cacheValue, StandardCharsets.UTF_8);
        } else {
            ...
            cacheValue = byteStream.toByteArray();
        }
        ...
        try {
            ...
            final byte[] testValue = cache.get(cacheKey, keySerializer, valueDeserializer);
            logger.info("UpdateState: testing> " + testValue.toString());
